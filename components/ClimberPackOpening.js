...
export default function ClimberPackOpening({ athletes = [], maxCards = 5, onDone }) {
  ...
  const startAnimation = useCallback(() => {
    ...
    later(() => {
      cards = drawCards(athletes, Math.min(maxCards, athletes.length));
      setDrawn(cards);
      setPhase("cards");
  ...
  }, [athletes, clear, later, maxCards, onDone]);
  ...
  return (
    <div className="cr-root">
      ...
      <div className={`cr-pack ${phase}`} onClick={() => isClickable && startAnimation()} role="button" aria-label="Open climber pack">
      ...
      {phase === "idle" && <p className="cr-cta cr-cta-pulse">TAP TO OPEN PACK</p>}
      {phase === "done" && <p className="cr-cta" onClick={startAnimation} style={{ cursor: "pointer" }}>↻ OPEN ANOTHER PACK</p>}
      {phase === "done" && revealed.length === 0 && <p className="cr-hint">tap a card to inspect</p>}
    </div>
  );
}
    ...
    >
      ...
      <div className="cr-card-face cr-card-front" style={{ "--card-bg-from": athlete.rarity === "legendary" ? "#3D2000" : athlete.rarity === "rare" ? "#001A3D" : "#1C1C1C" }}>
        ...
        <div className="cr-card-head">
          ...
          <span className="cr-discipline">{athlete.discipline}</span>
        </div>
        ...
        <div className="cr-art-zone">
          ...
          <span className="cr-art-emoji">{athlete.emoji || "🧗"}</span>
          <span className="cr-grade-overlay">{athlete.grade || "7a"}</span>
        </div>

        <div className="cr-name-row">
          <span className="cr-athlete-name">{athlete.name}</span>
          <span className="cr-flag">{athlete.flag || "🧗"} {athlete.country || "GYM"}</span>
        </div>

        <p className="cr-quote">&ldquo;{athlete.quote || "Keep climbing."}&rdquo;</p>

        <div className="cr-stats">
          <StatBar label="POW" value={athlete.stats.power} color={meta.color} />
          <StatBar label="TEC" value={athlete.stats.tech} color={meta.color} />
          <StatBar label="END" value={athlete.stats.endurance} color={meta.color} />
        </div>
...
function Particles() {
  ...
  return (
    <div className="cr-particles" aria-hidden>
      {pts.map((p, i) => (
        <div
          key={i}
...
.cr-art-emoji { font-size: clamp(22px, 5.5vw, 38px); filter: drop-shadow(0 0 10px rgba(0,0,0,.7)); position: relative; z-index: 2; }
.cr-grade-overlay { position: absolute; bottom: 4px; right: 6px; font-size: clamp(14px, 3vw, 20px); font-weight: 700; color: var(--r-color); text-shadow: 0 0 10px var(--r-color), 0 2px 4px rgba(0,0,0,.9); z-index: 3; line-height: 1; }
.cr-name-row { display: flex; justify-content: space-between; align-items: baseline; padding: 3px 6px 1px; }
.cr-athlete-name { font-size: clamp(9px, 1.9vw, 12px); font-weight: 700; color: #fff; letter-spacing: .04em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 68%; }
.cr-flag { font-size: clamp(7px, 1.3vw, 9px); color: rgba(255,255,255,.5); font-weight: 600; letter-spacing: .05em; white-space: nowrap; }
...
.cr-stat-label { font-size: clamp(5.5px, 1.1vw, 7.5px); color: rgba(255,255,255,.4); letter-spacing: .1em; width: 22px; }
...
.cr-stat-val { font-size: clamp(6px, 1.1vw, 8px); font-weight: 700; color: rgba(255,255,255,.55); width: 18px; text-align: right; }
...
.cr-cta { position: absolute; bottom: 5%; left: 50%; transform: translateX(-50%); z-index: 30; font-size: clamp(11px, 2.2vw, 14px); letter-spacing: .25em; color: rgba(255,255,255,.45); text-transform: uppercase; animation: fadeUp .5s ease forwards; white-space: nowrap; }
.cr-cta-pulse { animation: fadeUp .5s ease forwards, ctaPulse 2s ease infinite; }
@keyframes ctaPulse { 0%,100% { opacity: .3; } 50% { opacity: .75; } }
.cr-hint { position: absolute; bottom: 1.5%; left:50%; transform: translateX(-50%); z-index:30; font-size: clamp(8px,1.5vw,10px); letter-spacing:.18em; color:rgba(255,255,255,.2); text-transform:uppercase; animation: fadeUp .8s .4s ease both; white-space:nowrap; }
