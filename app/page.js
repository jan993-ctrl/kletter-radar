...
// --- EIGENE KOMPONENTE FÜR DIE KARTE (wegen Hover-State) ---
    ...
    >
      ...
      >
        ...
        <div style={{ ...cardFrontStyle, background: tierTheme.frontBg, border: tierTheme.border }}>
          ...
          {/* BILD-CONTAINER MIT HOVER-ZOOM */}
          <div style={imgContainerStyle}>
            <Image
              src={c.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(c.name || "User")}&background=random&size=300`}
              style={{
                ...imgStyle,
                // Wenn Hover: Zoom rein und Fokus nach oben schieben (Headshot)
                // Wenn kein Hover: Ganze Ansicht (contain/cover Mix)
                transform: isHovered ? "scale(1.4) translateY(10%)" : "scale(1.0) translateY(0%)",
              ...
              unoptimized
            />
            {/* Overlay-Schatten für bessere Lesbarkeit des Namens */}
            <div style={{
              position: 'absolute',
              bottom: 0,
              left: 0,
                ...
                <section style={gridPanelStyle}>
                  <div style={gridStyle}>
                    {hasLocalContext
                      ...
                      : <div style={emptyStateStyle}>Heimathalle festlegen...</div>}
                  </div>
                </section>
...
const mainStyle = { fontFamily: "'Inter', sans-serif", maxWidth: "1200px", margin: "0 auto", color: "#f4f4f5" };
...
const baseFaceStyle = { position: "absolute", width: "100%", height: "100%", backfaceVisibility: "hidden", borderRadius: "26px", border: "1px solid rgba(255,255,255,0.14)", boxShadow: "0 24px 55px rgba(2,6,23,0.6), inset 0 1px 0 rgba(255,255,255,0.08)", overflow: "hidden" };
...
const imgContainerStyle = { 
  position: "relative", 
  height: "230px", // Etwas höher für mehr Bildfläche
  width: "100%", 
  backgroundColor: "#000",
  overflow: "hidden" // Wichtig, damit das Bild beim Zoomen nicht über die Karte ragt
};
...
const imgStyle = { 
  ...
  transition: "transform 0.5s cubic-bezier(0.165, 0.84, 0.44, 1), object-position 0.5s ease" 
};
